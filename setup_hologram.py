...
files = glob.glob('streamlit-app/ui/*.py')
for f in files:
    with open(f, 'r') as file:
        content = file.read()
    
    start_idx = content.find("st.markdown('''<style>")
    end_idx = content.find("</style>''', unsafe_allow_html=True)")
    
    if start_idx != -1 and end_idx != -1:
        new_content = content[:start_idx] + HOLO_CSS.strip() + content[end_idx + len("</style>''', unsafe_allow_html=True)"):]
        with open(f, 'w') as file:
            file.write(new_content)
print("Holographic UI Applied!")
